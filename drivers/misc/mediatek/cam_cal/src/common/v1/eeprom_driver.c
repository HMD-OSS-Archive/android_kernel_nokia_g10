...
#endif
{
	...
	case CAM_CALIOC_G_READ:
        ...
        if((ptempbuf->deviceID == 0x01)&&(g_eepromMainData != NULL)){
        ...
        } else if((ptempbuf->deviceID == 0x02) && (ptempbuf->sensorID == WR_HI556_FRONT_LY_SENSOR_ID)) {
            if(ptempbuf->u4Offset == 1){
            ...
            i4RetValue = ptempbuf->u4Length;
        } else if((ptempbuf->deviceID == 0x02) && (ptempbuf->sensorID == WR_GC5035_FRONT_QH_SENSOR_ID)) {
            if(ptempbuf->u4Offset == 1){
...
//+bug584789 chenbocheng.wt, add, 2020/10/30, add main camera s5kgm1 and wide camera hi556 otp code
        ...
        if((ptempbuf->deviceID == 0x01)&&(g_eepromMainData != NULL)){
        ...
        }else if(ptempbuf->deviceID == 0x02 &&ptempbuf->sensorID == RONIN_HI846_FRONT_ST_SENSOR_ID) {
            if(ptempbuf->u4Offset == 0x201 && ptempbuf->u4Length == 0x04){ //check vendor id
                ...
                pr_debug("%d,hi846,ifCAM_CALIOC_G_READ start! offset=0x%x, length=0x%x,pu1Params:0x%x,sensorid:0x%x\n",__LINE__,
                    ptempbuf->u4Offset, ptempbuf->u4Length,*pu1Params,ptempbuf->sensorID);
                if (ptempbuf->sensorID == RONIN_HI846_FRONT_ST_SENSOR_ID && ptempbuf->u4Length == 0x07 && ptempbuf->u4Offset == 0x201){//HI846 MID data
                    memcpy(pu1Params,(u8 *)&hi846_module_id,ptempbuf->u4Length);
                }
                if (ptempbuf->sensorID == RONIN_HI846_FRONT_ST_SENSOR_ID && ptempbuf->u4Length == 0x10 && ptempbuf->u4Offset == 0x21A && hi846_awb_valid){//HI846 AWB data
                    memcpy(pu1Params,(u8 *) &hi846_data_awb[0],ptempbuf->u4Length);
                }
                if (ptempbuf->sensorID == RONIN_HI846_FRONT_ST_SENSOR_ID && ptempbuf->u4Length == 0x74C && ptempbuf->u4Offset == 0x24E && hi846_lsc_valid){//HI846 LSC data
                    memcpy(pu1Params,(u8 *) &hi846_data_lsc[0],ptempbuf->u4Length);
                }
            }
            i4RetValue = ptempbuf->u4Length;
        } else if((ptempbuf->deviceID == 0x08) && (ptempbuf->sensorID == RONIN_HI556_WIDE_DC_SENSOR_ID)) {
            if(ptempbuf->u4Offset == 1){
                memcpy(pu1Params,(u8 *)&hi556_vendor_id,4);
            }else{
                pr_debug("%d,hi556,ifCAM_CALIOC_G_READ start! offset=0x%x, length=0x%x,pu1Params:0x%x,sensorid:0x%x\n",__LINE__,
                            ptempbuf->u4Offset, ptempbuf->u4Length,*pu1Params,ptempbuf->sensorID);
                if (ptempbuf->sensorID == RONIN_HI556_WIDE_DC_SENSOR_ID && ptempbuf->u4Length == 0x07 && ptempbuf->u4Offset == 0x40B){//HI846 MID data
                    memcpy(pu1Params,(u8 *)&hi556_module_id,ptempbuf->u4Length);
                }
                if (ptempbuf->sensorID == RONIN_HI556_WIDE_DC_SENSOR_ID && ptempbuf->u4Length == 0x10 && ptempbuf->u4Offset == 0x41b && hi556_awb_valid){//HI556 AWB data
                    memcpy(pu1Params,(u8 *) &hi556_data_awb[0],ptempbuf->u4Length);
                }
                if (ptempbuf->sensorID == RONIN_HI556_WIDE_DC_SENSOR_ID && ptempbuf->u4Length == 0x74C && ptempbuf->u4Offset == 0x44f && hi556_lsc_valid){//HI556 LSC data
                    memcpy(pu1Params,(u8 *) &hi556_data_lsc[0],ptempbuf->u4Length);
                }
            }
            i4RetValue = ptempbuf->u4Length;
        } else if((ptempbuf->deviceID == 0x08) && (ptempbuf->sensorID == RONIN_GC5035_WIDE_ST_SENSOR_ID) && (g_eepromMainWideData != NULL)) {  //+bug596770,liuxingyu.wt,ADD,2020/11/10, add gc5035 cam eeprom
            u32 totalLength = ptempbuf->u4Offset+ ptempbuf->u4Length;
        ...
        } else if((ptempbuf->deviceID == 0x02) && (ptempbuf->sensorID == RONIN_GC8034_FRONT_LH_SENSOR_ID) && (g_eepromSubData != NULL)){   //+bug596770,liuxingyu.wt,ADD,2020/11/10, add gc8034 cam eeprom
            u32 totalLength = ptempbuf->u4Offset+ ptempbuf->u4Length;
...
//+bug584789,zhouyikuan.wt,ADD,2020/20/28,HI846 eeprom bring up
                 ...
                 //-bug596770,houzhipeng.wt,ADD,2020/11/03, add macro cam eeprom
                         ...
                         //+bug584789,zhouyikuan.wt,ADD,2020/11/04,GC8034 eeprom bring up
                         if(ptempbuf->sensorID == ROGUE_HI846_FRONT_ST_SENSOR_ID){
                             if(ptempbuf->u4Offset == 0x201 && ptempbuf->u4Length == 0x04){ //check vendor id
                                     memcpy(pu1Params,(u8 *)&hi846_vendor_id,4);
                             }else{
                           ...
                           i4RetValue = ptempbuf->u4Length;
                        }else if(ptempbuf->sensorID == ROGUE_GC8034_FRONT_LH_SENSOR_ID){
                u32 totalLength = ptempbuf->u4Offset+ ptempbuf->u4Length;
                LOG_INF("read otp in macro totalLength:%d, dataLength:%d\n",totalLength,g_eepromSubData->dataLength);
                if((g_eepromSubData->dataBuffer)&&(totalLength <= g_eepromSubData->dataLength)){
                    if(ptempbuf->u4Offset == 1){
