...
#include "ronin_gc8034_front_lh_mipi_Sensor.h"
...
static kal_uint16 read_rogue_gc8034_eeprom(kal_uint32 addr)
{
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = RONIN_GC8034_FRONT_LH_SENSOR_ID,
...
static struct stCAM_CAL_DATAINFO_STRUCT ronin_gc8034_front_lh_eeprom_data ={
    .sensorID= RONIN_GC8034_FRONT_LH_SENSOR_ID,
    .deviceID = 0x02,
    .dataLength = 0x76A,
    .sensorVendorid = 0x1A0501ff,
    .vendorByte = {1,2,3,4},
    .dataBuffer = NULL,
};
static struct stCAM_CAL_CHECKSUM_STRUCT ronin_gc8034_front_lh_checksum[8] =
{
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				//+bug584789,zhouyikuan.wt,ADD,2020/11/04,GC8034 eeprom bring up
				size = imgSensorReadEepromData(&ronin_gc8034_front_lh_eeprom_data,ronin_gc8034_front_lh_checksum);
				if(size != ronin_gc8034_front_lh_eeprom_data.dataLength || (ronin_gc8034_front_lh_eeprom_data.sensorVendorid >> 24)!= read_rogue_gc8034_eeprom(0x0001)) {
					pr_err("get eeprom data failed\n");
					if(ronin_gc8034_front_lh_eeprom_data.dataBuffer != NULL) {
						kfree(ronin_gc8034_front_lh_eeprom_data.dataBuffer);
						ronin_gc8034_front_lh_eeprom_data.dataBuffer = NULL;
					}
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				} else {
					pr_info("get eeprom data success\n");
					imgSensorSetEepromData(&ronin_gc8034_front_lh_eeprom_data);
				}
                //-bug584789,zhouyikuan.wt,ADD,2020/11/04,GC8034 eeprom bring up
                return ERROR_NONE;
			}
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			pr_err("[gc8034_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
UINT32 RONIN_GC8034_FRONT_LH_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
