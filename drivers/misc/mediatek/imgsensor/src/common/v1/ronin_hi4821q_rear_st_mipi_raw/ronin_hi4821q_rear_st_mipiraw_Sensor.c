...
kal_uint8 IsNewVER = 0;


#define HI4821Q_CUSTOM2_3_ENABLE 0
...
#define PGC_BLOCK_X  13  //33(old sensor)
#define PGC_BLOCK_Y  11  //25(old sensor)

// SRAM Information
#define SRAM_XGC_START_ADDR_48M     0x43F0
#define SRAM_QGC_START_ADDR_48M     0x4000
#define SRAM_PGC_START_ADDR_48M     0x5068  //0x8980 (2020.9.14)
...
static kal_uint16 XGC_setting_burst[XGC_DATA_SIZE*2];

static kal_uint16 BPGC_setting_burst[BPGC_DATA_SIZE*2];
...
static void apply_sensor_XGC_QGC_Cali(void)
{
	...
	//write_cmos_sensor(0x0b04,0xd560); //XGC, QGC, Q2B disable
	...
	//XGC data apply
	{
		...
		for(i = 0; i < XGC_BLOCK_X*XGC_BLOCK_Y*10; i += 2) //9(BLOCK_X)* 7 (BLCOK_Y)*10(channel)
		{
			...
			XGC_setting_burst[2*length]=sensor_xgc_addr;
			XGC_setting_burst[2*length+1]=hi4821_xgc_data;
			length++;
...
static void apply_sensor_BPGC_Cali(void)
{
	...
	//BPGC data apply
	{
		...
		sensor_bpgc_addr = SRAM_PGC_START_ADDR_48M;
		for(i = 0; i < PGC_BLOCK_X*PGC_BLOCK_Y*4;i+=2) //13(BLOCK_X)*11(BLCOK_Y)*2(channel)*2bytes
		{
			hi4821_bpgc_data = ((((pgc_data_buffer[i+1]) & (0x00ff)) <<8) + ((pgc_data_buffer[i]) & (0x00ff)));
			BPGC_setting_burst[2*length]=sensor_bpgc_addr;
			BPGC_setting_burst[2*length+1]=hi4821_bpgc_data;
			length++;
			//write_cmos_sensor(sensor_bpgc_addr,hi4821_bpgc_data);

			#if HI4821Q_OTP_DUMP
			LOG_INF("sensor_bpgc_addr:0x%x,pgc_data_buffer[%d]:0x%x,pgc_data_buffer[%d]:0x%x,hi4821_bpgc_data:0x%x\n",
				sensor_bpgc_addr,i,pgc_data_buffer[i],i+1,pgc_data_buffer[i+1],hi4821_bpgc_data);
			#endif
			sensor_bpgc_addr += 2;
			idx++;

			if(length==BPGC_DATA_SIZE/2)
			{
...
kal_uint16 addr_data_pair_init1_hi4821q[] = {
     0x0790, 0x0100,
     0x9000, 0x706F,
     0x9002, 0x0000,
     0x9004, 0x67B1,
...
{
	hi4821q_table_write_cmos_sensor(
		addr_data_pair_init1_hi4821q,
		sizeof(addr_data_pair_init1_hi4821q) /
		sizeof(kal_uint16));
...
kal_uint16 addr_data_pair_preview1_hi4821q[] = {
	0x0B00, 0x0000,
	0xC298, 0x0042,
	0xC29A, 0x0047,
	0xC29C, 0x0074,
...
{
	hi4821q_table_write_cmos_sensor(
		addr_data_pair_preview1_hi4821q,
		sizeof(addr_data_pair_preview1_hi4821q) /
		sizeof(kal_uint16));
}
...
kal_uint16 addr_data_pair_capture1_30fps_hi4821q[] = {
	0x0B00, 0x0000,
	0xC298, 0x0042,
	0xC29A, 0x0047,
	0xC29C, 0x0074,
...
{
	hi4821q_table_write_cmos_sensor(
		addr_data_pair_capture1_30fps_hi4821q,
		sizeof(addr_data_pair_capture1_30fps_hi4821q) /
		sizeof(kal_uint16));
...
kal_uint16 addr_data_pair_video1_hi4821q[] = {
	0x0B00, 0x0000,
	0xC298, 0x0042,
	0xC29A, 0x0047,
	0xC29C, 0x0074,
	...
	0x0F0E, 0x6677,
	0x0B04, 0x90F4,
	0x0B20, 0x0FA0,// x_output_size
	0x0B22, 0x0BB8,// y_output_size
	0x1100, 0x1141,
	0x1130, 0x0000,
	0x1E00, 0x0004,
	0x1702, 0x1C9C,
...
{
	hi4821q_table_write_cmos_sensor(
		addr_data_pair_video1_hi4821q,
		sizeof(addr_data_pair_video1_hi4821q) /
		sizeof(kal_uint16));
...
kal_uint16 addr_data_pair_hs_video1_hi4821q[] = {
	0x0B00, 0x0000,
	0xC298, 0x0042,
	0xC29A, 0x0047,
	0xC29C, 0x0074,
...
{
	hi4821q_table_write_cmos_sensor(
		addr_data_pair_hs_video1_hi4821q,
		sizeof(addr_data_pair_hs_video1_hi4821q) /
		sizeof(kal_uint16));
...
kal_uint16 addr_data_pair_slim_video_hi4821q[] = {
	...
	0x1048, 0x00F8,
	 ...
	 kal_uint16 addr_data_pair_slim_video1_hi4821q[] = {
	0x0B00, 0x0000,
	0xC298, 0x0042,
	0xC29A, 0x0047,
	0xC29C, 0x0074,
...
{
	hi4821q_table_write_cmos_sensor(
		addr_data_pair_slim_video1_hi4821q,
		sizeof(addr_data_pair_slim_video1_hi4821q) /
		sizeof(kal_uint16));
}
...
kal_uint16 addr_data_pair_custom11_hi4821q[] = {
	0x0B00, 0x0000,
	0xC298, 0x0042,
	0xC29A, 0x0047,
	0xC29C, 0x0074,
...
{
	hi4821q_table_write_cmos_sensor(
		addr_data_pair_custom11_hi4821q,
		sizeof(addr_data_pair_custom11_hi4821q) /
		sizeof(kal_uint16));
...
kal_uint16 addr_data_pair_custom2_hi4821q[] = {
	...
	0x1048, 0x00FA,
	 ...
	 kal_uint16 addr_data_pair_custom22_hi4821q[] = {
	0x0B00, 0x0000,
	0xC298, 0x07FF,
	0xC29A, 0x07FF,
	0xC29C, 0x07FF,
...
{
	hi4821q_table_write_cmos_sensor(
		addr_data_pair_custom22_hi4821q,
		sizeof(addr_data_pair_custom22_hi4821q) /
		sizeof(kal_uint16));
...
kal_uint16 addr_data_pair_custom33_hi4821q[] = {
	0x0B00, 0x0000,
	0xC298, 0x07FF,
	0xC29A, 0x07FF,
	0xC29C, 0x07FF,
...
{
		 hi4821q_table_write_cmos_sensor(
			 addr_data_pair_custom33_hi4821q,
			 sizeof(addr_data_pair_custom33_hi4821q) /
			 sizeof(kal_uint16));
}
...
static struct stCAM_CAL_CHECKSUM_STRUCT st_rear_hi4821q_Checksum_old[11] =
{
...
//+bug584789 lintaicheng.wt, add, 2021/01/15, add main camera new hi4821q otp code
static struct stCAM_CAL_CHECKSUM_STRUCT st_rear_hi4821q_Checksum_new[12] =
{
	{MODULE_ITEM,0x0000,0x0000,0x0007,0x0008,0x55},
	{AWB_ITEM,0x0009,0x0009,0x0019,0x001A,0x55},
	{AF_ITEM,0x001B,0x001B,0x0020,0x0021,0x55},
	...
	{PDAF_ITEM,0x0770,0x0770,0x0960,0x0961,0x55},
	{PDAF_PROC2_ITEM,0x0962,0x0962,0x0D4E,0x0D4F,0x55},
	{HI4821Q_PGC,0x0D50,0x0D50,0x0F8C,0x0F8D,0x55},
	{HI4821Q_QGC,0x0F8E,0x0F8E,0x1380,0x1381,0x55},
	{HI4821Q_XGC,0x1382,0x1382,0x15FA,0x15FB,0x55},
	{TOTAL_ITEM,0x0000,0x0000,0x163B,0x163C,0x55},
	{MAX_ITEM,0xFFFF,0xFFFF,0xFFFF,0xFFFF,0x55},  // this line must haved
};
//-bug584789 lintaicheng.wt, add, 2021/01/15, add main camera new hi4821q otp code
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	kal_int16 RevID = 0;
	//bug584789 lintaicheng.wt, add, 2021/01/15, add main camera new hi4821q otp code
	struct stCAM_CAL_CHECKSUM_STRUCT *st_rear_hi4821q_Checksum;

	write_cmos_sensor(0x071e, 0x0542);
	RevID=(read_cmos_sensor(0x0718)<<8)|(read_cmos_sensor(0x0719));
	//bug584789 lintaicheng.wt, add, 2021/01/15, add main camera new hi4821q otp code
	st_rear_hi4821q_Checksum = st_rear_hi4821q_Checksum_old;
	if(RevID==0x0001)
	{
		IsNewVER=0;
		LOG_INF("It is old ver\n");
	}
	else if(RevID==0x0101)
	{
		IsNewVER=1;
		//+bug584789 lintaicheng.wt, add, 2021/01/15, add main camera new hi4821q otp code
		st_rear_hi4821q_eeprom_data.dataLength = 0x163D;
		st_rear_hi4821q_Checksum = st_rear_hi4821q_Checksum_new;
		//-bug584789 lintaicheng.wt, add, 2021/01/15, add main camera new hi4821q otp code
		LOG_INF("It is new ver\n");
	}
	else
	{
        	LOG_INF("It is not a valid ver\n");
	}
	write_cmos_sensor(0x071e, 0000);
			...
			if(size != st_rear_hi4821q_eeprom_data.dataLength || (st_rear_hi4821q_eeprom_data.sensorVendorid >> 24)!= hi4821q_read_eeprom(0x0001)) {
				...
				*sensor_id = 0xFFFFFFFF;
				...
				imgSensorSetEepromData(&st_rear_hi4821q_eeprom_data);
					...
					pgc_data_buffer = &(st_rear_hi4821q_eeprom_data.dataBuffer[st_rear_hi4821q_Checksum[HI4821Q_PGC - 1].startAdress + 1]);
					...
					xgc_data_buffer = &(st_rear_hi4821q_eeprom_data.dataBuffer[st_rear_hi4821q_Checksum[HI4821Q_XGC - 1].startAdress + 3]);
						...
						pr_info("hi4821q_eeprom:bpgc_addr:0x%x\n",st_rear_hi4821q_Checksum[HI4821Q_PGC - 1].startAdress + 1);
						...
						pr_info("=====================hi4821q dump bpgc eeprom data start====================\n");
						dumpEEPROMData(BPGC_DATA_SIZE,pgc_data_buffer);
						pr_info("=====================hi4821q dump bpgc eeprom data end======================\n");
