...
#include "ronin_hi556_wide_dcmipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = RONIN_HI556_WIDE_DC_SENSOR_ID,
...
#define MODULE_GROUP1_FLAG 0x401
#define AWB_GROUP1_FLAG 0x41C
#define LSC_GROUP1_FLAG 0x452
...
static int read_hi556_module_info(void)
{
	...
	/* read flag */
	for(i = 0; i < 3; i++){
		write_cmos_sensor_8(0x010a,(((MODULE_GROUP1_FLAG)+(MODULE_INFO_SIZE + 2)*i)>>8)&0xff);
		write_cmos_sensor_8(0x010b,((MODULE_GROUP1_FLAG)+(MODULE_INFO_SIZE + 2)*i)&0xff);
		write_cmos_sensor_8(0x0102,0x01);
		otp_grp_flag = read_cmos_sensor(0x0108);

		if(otp_grp_flag == VALID_OTP_FLAG){
			minfo_start_addr = (MODULE_GROUP1_FLAG) + (MODULE_INFO_SIZE + 2)*i + 1;
			pr_info("the group:%i is valid,otp_grp_flag = 0x%x,info_start_addr:0x%x\n",i+1,otp_grp_flag,minfo_start_addr);
			break;
		}
	}
	if(otp_grp_flag != VALID_OTP_FLAG){
		pr_err("no OTP hi556_data_info\n");
		return 0;
	}
	check_sum_cal += VALID_OTP_FLAG;
...
#if HI556_OTP_DUMP
	dumpEEPROMData(MODULE_INFO_SIZE,&hi556_data_info[0]);
#endif
	//LOG_INF("module_id=0x%x position=0x%x\n", hi556_module_id, position);
	pr_info("=== HI556 INFO module_id=0x%x position=0x%x ===\n", hi556_module_id, position);
	pr_info("=== HI556 INFO lens_id=0x%x,vcm_id=0x%x ===\n",lens_id, vcm_id);
	pr_info("=== HI556 INFO date is %d-%d-%d ===\n",year,month,day);
	pr_info("=== HI556 INFO check_sum=0x%x,check_sum_cal=0x%x ===\n", check_sum, check_sum_cal);
...
static int read_hi556_awb_info(void)
{
	...
	/* awb group 1 */
	for(i = 0; i < 3; i++){
		write_cmos_sensor_8(0x010a,(((AWB_GROUP1_FLAG)+(AWB_DATA_SIZE + 2)*i)>>8)&0xff);
		write_cmos_sensor_8(0x010b,((AWB_GROUP1_FLAG)+(AWB_DATA_SIZE + 2)*i)&0xff);
		write_cmos_sensor_8(0x0102,0x01);
		otp_grp_flag = read_cmos_sensor(0x0108);

		if(otp_grp_flag == VALID_OTP_FLAG){
			awb_start_addr = (AWB_GROUP1_FLAG) + (AWB_DATA_SIZE + 2)*i + 1;
			pr_info("the group:%i is valid,otp_grp_flag = 0x%x,awb_start_addr:0x%x\n",i+1,otp_grp_flag,awb_start_addr);
			break;
		}
	}
	if(otp_grp_flag != VALID_OTP_FLAG){
	...
	check_sum_awb_cal += VALID_OTP_FLAG;
	if(awb_start_addr != 0)
	{
		...
		pr_info("check_sum_awb_cal =0x%x \n",check_sum_awb_cal);
...
#if HI556_OTP_DUMP
	dumpEEPROMData(AWB_DATA_SIZE,&hi556_data_awb[0]);
#endif
	pr_info("=== HI556 AWB r=0x%x, b=0x%x, gr=%x, gb=0x%x ===\n", r, b,gb, gr);
	pr_info("=== HI556 AWB gr=0x%x,gb=0x%x,gGr=%x, gGb=0x%x ===\n", golden_r, golden_b, golden_gr, golden_gb);
	pr_info("=== HI556 AWB check_sum_awb=0x%x,check_sum_awb_cal=0x%x ===\n",check_sum_awb,check_sum_awb_cal);
...
static int read_hi556_lsc_info(void)
{
	...
	/* lsc group 1 */
	for(i = 0; i < 3; i++){
		write_cmos_sensor_8(0x010a,(((LSC_GROUP1_FLAG)+(LSC_DATA_SIZE + 2)*i)>>8)&0xff);
		write_cmos_sensor_8(0x010b,((LSC_GROUP1_FLAG)+(LSC_DATA_SIZE + 2)*i)&0xff);
		write_cmos_sensor_8(0x0102,0x01);
		otp_grp_flag = read_cmos_sensor(0x0108);

		if(otp_grp_flag == VALID_OTP_FLAG){
			lsc_start_addr = (LSC_GROUP1_FLAG) + (LSC_DATA_SIZE + 2)*i + 1;
			pr_info("the group:%i is valid,otp_grp_flag:0x%x,lsc_start_addr:0x%x\n",i+1,otp_grp_flag,lsc_start_addr);
			break;
		}
	}
	if(otp_grp_flag != VALID_OTP_FLAG){
...
#if HI556_OTP_DUMP
	dumpEEPROMData(LSC_DATA_SIZE,&hi556_data_lsc[0]);
#endif
	pr_info("=== HI556 LSC check_sum_lsc=0x%x, check_sum_lsc_cal=0x%x ===\n", check_sum_lsc, check_sum_lsc_cal);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_info(
					"liuxingyu get_imgsensor_id i2c write id : 0x%x, sensor id: 0x%x\n",
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		pr_info("liuxingyu get_imgsensor_id Read id fail,sensor id: 0x%x\n", *sensor_id);
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	return ERROR_NONE;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_info("liuxingyu open i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
	...
	if (imgsensor_info.sensor_id != sensor_id) {
		pr_err("liuxingyu open %s sensor id fail: 0x%x\n", __func__, sensor_id);
		return ERROR_SENSOR_CONNECT_FAIL;
	}
...
UINT32 RONIN_HI556_WIDE_DC_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
