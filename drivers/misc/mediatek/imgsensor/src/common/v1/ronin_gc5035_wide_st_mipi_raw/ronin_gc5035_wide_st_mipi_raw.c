...
#include "ronin_gc5035_wide_st_mipi_raw.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = RONIN_GC5035_WIDE_ST_SENSOR_ID,
...
static struct stCAM_CAL_DATAINFO_STRUCT ronin_gc5035_wide_st_eeprom_data ={
	.sensorID= RONIN_GC5035_WIDE_ST_SENSOR_ID,
	.deviceID = 0x08,
	.dataLength = 0x076A,
	.sensorVendorid = 0x17030001,
	.vendorByte = {1,2,3,4},
	.dataBuffer = NULL,
};
static struct stCAM_CAL_CHECKSUM_STRUCT ronin_gc5035_wide_st_checksum[8] =
{
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				gc5035_otp_identify();
				//+bug577281 chenbocheng.wt, add, 2020/9/9, camera otp porting
				size = imgSensorReadEepromData(&ronin_gc5035_wide_st_eeprom_data,ronin_gc5035_wide_st_checksum);
				if((size != ronin_gc5035_wide_st_eeprom_data.dataLength) || ((ronin_gc5035_wide_st_eeprom_data.sensorVendorid >> 24)!= read_eeprom(0x0001))) {
					pr_err("gc5035 get eeprom data failed\n");
					if(ronin_gc5035_wide_st_eeprom_data.dataBuffer != NULL) {
						kfree(ronin_gc5035_wide_st_eeprom_data.dataBuffer);
						ronin_gc5035_wide_st_eeprom_data.dataBuffer = NULL;
					}
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				} else {
					pr_info("get eeprom data success\n");
					imgSensorSetEepromData(&ronin_gc5035_wide_st_eeprom_data);
				}
				//-bug577281 chenbocheng.wt, add, 2020/9/9, camera otp porting
...
UINT32 RONIN_GC5035_WIDE_ST_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
