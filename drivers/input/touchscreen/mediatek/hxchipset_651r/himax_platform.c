...
const struct of_device_id himax_match_table[] = {
	{.compatible = "mediatek,himax_common" },
	{},
};
...
{
//+bug581495, sheqihao.wt, MODIFY, 20200825, Remove the Power Supply VLDO28 that TP no need
	/*int error = 0;
...
//+bug558651, sheqihao.wt, MODIFY, 20200604, fix tp abnormal after esd test 
int himax_ts_suspend_func(void)
{
	struct himax_ts_data *ts = private_ts;
	queue_work(ts->ts_susned_workqueue, &ts->ts_suspend_work);
	return 0;
}

EXPORT_SYMBOL(himax_ts_suspend_func);

int himax_ts_recovery_func(void)
{
	struct himax_ts_data *ts = private_ts;
	queue_work(ts->ts_recovery_workqueue, &ts->ts_recovery_work);
	return 0;
}

EXPORT_SYMBOL(himax_ts_recovery_func);
//-bug558651, sheqihao.wt, MODIFY, 20200604, fix tp abnormal after esd test 

//+WVR-4220 sheqihao.wt,Modify,20200730,fix phone screen freeze in call scene
#if defined(HX_LCM_RESUME_TS)
/********************************************************************************
*FUNCTION

*  himax_lcm_resume_func

*DESCRIPTION

*  lcm resume touch function
...
* NONE
********************************************************************************/
void himax_lcm_resume_func(void)
{
	struct himax_ts_data *ts = private_ts;
	queue_work(ts->ts_lcm_resume_workqueue, &ts->ts_lcm_resume_work);
	return;
}
#else
void himax_lcm_resume_func(void)
...
EXPORT_SYMBOL(himax_lcm_resume_func);
//-WVR-4220 sheqihao.wt,Modify,20200730,fix phone screen freeze in call scene
...
static int himax_common_local_init(void)
{
	...
	I("[Himax] 651_R Himax_ts SPI Touchscreen Driver local init\n");
